...
# --- Mapeamento de Portas Conhecidas para Nomes de Serviços ---
# Adicione ou remova portas conforme sua necessidade.
KNOWN_PORTS = {
    ...
    '27018': 'MongoDB Shard', '27019': 'MongoDB Config Server'
}

# --- Lista de Portas de Serviço de Interesse ---
# Filtra as conexões para focar apenas nas portas relevantes para análise de firewall.
...
SERVICE_PORTS_OF_INTEREST = {
    '22', '23', '25', '53', '80', '110', '143', '389', '443', '445', '587', '636', '993', '995',
    '1433', '3306', '3389', '5432', '5060', '5061', '8000', '8080', '8443',
    '27017', '27018', '27019' # Adicionado portas do MongoDB como exemplo
}

# --- Configurações de Rede para o Diagrama ---
# Ajuste estes valores para o seu ambiente de rede local.
# Usado para determinar se um IP é da rede local (ex: '192.168.1.' para /24).
LOCAL_HOST_IP_PREFIX = '192.168.1.' 
# O IP do seu gateway/firewall.
...
def collect_connection_data():
    ...
    try:
        # Adiciona encoding='latin-1' para tentar resolver problemas de caracteres especiais no Windows
...
def parse_connection_output(raw_output):
    """
    Analisa a saída bruta do comando de conexão e retorna uma lista de dicionários de conexões.
    Filtra conexões irrelevantes (loopback, estados não interessantes, portas fora da lista de interesse).
    """
    connections = []
    ...
    for i, line in enumerate(lines):
        ...
        if state == 'LISTENING':
            ...
            # Para conexões ESTABLISHED, a porta relevante é a *porta de serviço estrangeira*.
...
def generate_network_diagram(connections, filename="network_diagram", format="png", current_hostname="", current_host_ip=""):
    ...
    for conn in connections:
        ...
        # Resolução de Hostname para IPs Estrangeiros
        if foreign_ip_node and foreign_ip_node != '0.0.0.0' and foreign_ip_node != '*' and foreign_ip_node != '::' and foreign_ip_node != GATEWAY_IP and foreign_ip_node not in nodes:
            foreign_node_label = foreign_ip_node
            try:
            ...
            except (socket.herror, socket.timeout, Exception) as e:
    ...
    for conn in connections:
        ...
        if conn['state'] == 'ESTABLISHED':
            ...
            # Se a conexão é para o próprio gateway (tráfego direto para o roteador/firewall)
                ...
                label_connection = f"{conn['protocol']} {conn['foreign_port']}\n({conn['service_foreign']})"
                dot.edge(local_ip_node, GATEWAY_IP, label=label_connection, color='purple', penwidth='1.5') 
...
def main():
    ...
    if raw_output:
        ...
        # print(raw_output) # Descomente esta linha para depuração, se quiser ver a saída bruta do netstat/ss
